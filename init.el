...
;; take a look at https://github.com/chrisdone/emacs-haskell-config

(use-package haskell-mode
  :ensure
  :defer

  ;; :mode (("\\.x$" . haskell-mode)      ; Alex (lexical analyser generator for Haskell)
  ;;        ;;("\.[di]cl$"  . haskell-mode) ; Clean
  ;;        )
  ...
  (add-hook
   ...
   (lambda ()
     ...
     ;; Interaction modes:
     ;;
     ;; `interactive-haskell-mode' Interact with per-project GHCi processes
     ;;   through a REPL and directory-aware sessions.
     ;;
     ;; `inf-haskell-mode' Interact with a GHCi process using
     ;;   comint-mode. Deprecated.
     ;;
     ...
     ;; Indentation modes:
     ;;
     ;;     `haskell-indentation-mode' (default) Semi-intelligent
     ;;       semi-automatic indentation: <TAB> selects the next potential
     ;;       indentation position, <S-TAB> selects the previous one. If a
     ;;       block is selected you can use <TAB> to indent the block more
     ;;       and <S-TAB> to indent the block less.
     ;;
     ;;     `haskell-indent-mode' Semi-intelligent semi-automatic
     ;;       indentation: <TAB> cycles through all available indentation
     ;;       positions. With <RET>, <DEL> and <BACKSPACE> indentations can
     ;;       be set and deleted as if they were real tabs. Propose certain
     ;;       completions: repeating a function name after writing its
     ;;       signature, repeating a function name for pattern matching,
     ...
     ;;     displayed near-misses. Each near-miss is listed with a digit;
     ...
     ;; ;; Enables Haskell support for speedbar, giving a listing of each
     ;; ;; module and under each module: imports, instances, data types,
     ;; ;; classes, bindings.
     ;; (speedbar-add-supported-extension ".hs")

     ;; ;;; unicode support
     ;; ;;; Type ‘->’ and it is immediately replaced with ‘→’.
     ;; ;;; Use C-\ to toggle the input method.
     ;; ;;; To see a table of all key sequences use M-x describe-input-method RET haskell-unicode
     ;; ;;;
     ...
     ;; ;; (flymake-mode) ; on-the-fly syntax checker

     ;; (require 'haskell-debug nil 'noerror)
     ))

  ;; (use-package ghc
  ;;   ;; ghc-mod front-end for haskell-mode
  ;;   ;; installed via package manager
  ;;   :config
  ;;   (add-hook 'haskell-mode-hook 'ghc-init))

  
  ;; (define-key haskell-mode-map "\et" 'transpose-words)

  ;; ;; interactive block indentation: de/indent either the following
  ;; ;; nested block or, if a region is active while in transient mark
  ;; ;; mode, de/indent the active region
  ;; (define-key haskell-mode-map (kbd "C-,") 'haskell-move-nested-left)
  ;; (define-key haskell-mode-map (kbd "C-.") 'haskell-move-nested-right)
  
  ;; Cycle groups of import lines or return to point (with prefix arg)
  (define-key haskell-mode-map [f8] #'haskell-navigate-imports)

  ;; http://haskell.github.io/haskell-mode/manual/latest/Compilation.html#Compilation
  (define-key haskell-mode-map [(control c) (control c)] #'haskell-compile)
  (define-key haskell-cabal-mode-map [(control c) (control c)] #'haskell-compile)
  
  ;; (define-key haskell-mode-map [(control \`)] 'haskell-interactive-bring)
  ...
  ;;  '(haskell-tags-on-save t);; generate tags via hasktags after saving
  ;;  '(haskell-indentation-indent-leftmost 'both)
  ;;  '(haskell-indentation-show-indentations t)
  ;;  )

  ;; ;; Use cabal-dev for the GHCi session. Ensures our dependencies are in
  ;; ;; scope.
  ;; ;; (haskell-process-type 'cabal-dev)

  ;; ;; Notify at the end of running Cabal commands or generally things worth
  ;; ;; notifying
  ...
  ;; ;; Display \ and -> and such using symbols in fonts.  This may sound
  ;; ;; like a neat trick, but be extra careful: it changes the alignment and
  ;; ;; can thus lead to nasty surprises w.r.t layout.
  ;; ;(setq haskell-font-lock-symbols t)

  ;; ;; http://www.reddit.com/r/haskell/comments/23g9dv/oc_haskell_programming_font_with_ligatures/ch1srdy
  ;; ;; (let ((font "Ubuntu Mono"))
  ;; ;;   (set-default-font font nil t)
  ;; ;;   (set-fontset-font t '(8500 . 8800) font))
  ;; ;; (setq haskell-font-lock-symbols t)

  ;; ;; (when (require 'haskell-unicode-input-method nil 'noerror)
  ;; ;;   (add-hook
  ;; ;;    'haskell-mode-hook
  ...
  ;; ;; Use these rules for telling the aligner about Haskell. You can just
  ;; ;; use M-x align, which I like to bind to M-[.
  ;; (add-hook
  ;;  'align-load-hook
  ;;  (lambda ()
...
;;   ;; Use w3m browser to browse Haddock docs inside Emacs.
;;   :ensure haskell-mode
;;   :after haskell-mode
;;   :config
;;   (add-hook 'w3m-display-hook #'w3m-haddock-display)
;;   (setq haskell-w3m-haddock-dirs '("~/.cabal/share/doc/" "/usr/share/doc/"))
;;   (bind-keys :map haskell-mode-map
;;              ([(control ?c) (control ?d)] . haskell-w3m-open-haddock)
;;              )
;;   (eval-after-load'w3m
;;    '(bind-keys :map w3m-mode-map
;;                ([(control ?c) (control ?d)] . haskell-w3m-open-haddock)
;;                ([(meta ?.)] . w3m-haddock-find-tag)
;;                )
;;          ))
