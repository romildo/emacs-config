(add-to-list 'load-path "~/.emacs-config/plugins/tabbar")

;;------------------------------------------------------------------------------
;;wcy-swbuff
;;------------------------------------------------------------------------------
(when (require 'wcy-swbuff nil 'noerror)
  (require 'wcy-swbuff)
...
;; (global-set-key (kbd "C-x f") 'tabbar-forward)
;; (global-set-key (kbd "C-x b") 'tabbar-backward)
;; (global-set-key (kbd "C-x p") 'tabbar-forward-group)
;; (global-set-key (kbd "C-x n") 'tabbar-backward-group)
 ...
 Emacs buffer are those starting with “*”." 
   (list 
    (cond 
     ...
     ;;第一个与最后一个字母是*
     ( (and (string-equal "*" (substring (buffer-name) 0 1)) (string-equal "*" (substring (buffer-name) -1 nil)) ) 
      "Dired & Emacs Buffers" 
      )
     
     ;;第一个字母是* 结尾不是
     ( (and (string-equal "*" (substring (buffer-name) 0 1)) (not (string-equal "*" (substring (buffer-name) -1 nil))) ) 
      "Emacs Other Buffers" 
      )
