...
(setq scilab-shell-global-key "\C-cs"
...
(if running-xemacs
  (progn
	(add-menu-button 
	 '("Tools") "---" )
	(add-menu-button 
	 '("Tools") ["Recompile scilab.el" scilab-recompile t ] )
	(add-menu-button 
	 '("Tools") ["Scilab Start" scilab-shell t] )
	(add-menu-button 
	 '("Tools") ["Scilab Setup" scilab-mode-setup t] )
	(add-menu-button 
	 '("Help") ["Scilab Topic" scilab-help t] )
	(add-menu-button 
	 '("Help") ["Scilab Help" scilab-help-function t] )
	(add-menu-button 
	 '("Help") ["Scilab Apropos" scilab-apropos-function t] ))

     (define-key menu-bar-tools-menu [separator-scilab]
       '("--"))
     (define-key menu-bar-tools-menu [scilab-recomp] 
       '("Recompile scilab.el"  . scilab-recompile))
     (define-key menu-bar-tools-menu [scilab-start] 
       '("Scilab Start"  . scilab-shell))
     (define-key menu-bar-tools-menu [scilab-setup] 
       '("Scilab Setup"  . scilab-mode-setup))

    (define-key menu-bar-help-menu [separator-scilab]
      '("--"))
    (define-key menu-bar-help-menu [scilab-apropos] 
      '("Scilab Apropos"  . scilab-apropos-function))
     (define-key menu-bar-help-menu [scilab-help] 
       '("Scilab Help"  . scilab-help-function))
     (define-key menu-bar-help-menu [scilab-topic] 
       '("Scilab Topic"  . scilab-help)))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
...
;;     2. Customize emacs-variable "gnuserv-program" to the path where executable
...
;;"/usr/lib/xemacs-21.1.12/i386-redhat-linux/gnuserv"
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(if running-gnuemacs 
    (progn
 ;      (defcustom scilab-gnuserv-program-path nil
; 	"Path for gnuserv program, need only for  emacs"
; 	:group 'scilab-shell
; 	:group 'scilab-setup
; 	:type 'string)
...
;       '(gnuserv-program scilab-gnuserv-program-path))

;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;Interface to emacs calculator
;;; Mode settings stored by Calc on Mon Jul  9 18:29:22 2001
(setq calc-complex-format 'I)
(setq calc-break-vectors t)
(setq calc-matrix-brackets '(O C))
(setq calc-angle-mode 'rad)
(setq calc-language 'maple)
